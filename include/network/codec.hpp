...
class Codec {
public:
    // Constructor now takes Channel reference
    ...
    // Deserializes a message frame from an input stream, adds source_id, and pushes to channel
    MessageFrame deserialize(std::istream& input, const std::string& source_id);

private:
    // The encryption/decryption key
    std::vector<uint8_t> key_;
    // Reference to the channel for message production
    Channel& channel_;
