...
/**
 * Test: StreamingAccess
 * Purpose: Tests the store's streaming access capabilities without loading entire file.
 * 
 * Methodology:
 * 1. Creates a large test file
 * 2. Opens a stream to read it
 * 3. Verifies stream operations work correctly
 * 
 * Expected Results:
 * - Stream should open successfully
 * - Content should match original
 * - Memory usage should remain constant
 */
TEST_F(StoreTest, StreamingAccess) {
    const std::string test_key = "stream_test";
    const std::string test_data = "Hello, Stream Test!";

    // Store test data
    auto input = create_test_stream(test_data);
    store->store(test_key, *input);

    // Get streaming access
    std::unique_ptr<std::istream> stream;
    ASSERT_NO_THROW({
        stream = store->get_stream(test_key);
    }) << "Opening stream should not throw";

    ASSERT_TRUE(stream && stream->good()) << "Stream should be valid";

    // Read and verify content
    std::string content;
    content.resize(test_data.size());
    ASSERT_TRUE(stream->read(&content[0], test_data.size())) << "Reading from stream failed";
    EXPECT_EQ(content, test_data) << "Stream content should match original";

    // Verify seeking works
    ASSERT_TRUE(stream->seekg(0)) << "Seeking to start should succeed";
    char first_char;
    ASSERT_TRUE(stream->get(first_char)) << "Reading first character should succeed";
    EXPECT_EQ(first_char, test_data[0]) << "First character should match";
}