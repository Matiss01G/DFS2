...
TEST_F(CryptoStreamTest, DecryptionMarkerHandling) {
    const std::string plaintext = "Hello, World! This is a test of stream encryption with marker.";
    std::stringstream input(plaintext);
    std::stringstream encrypted;
    std::stringstream decrypted;

    // Encrypt (should not add marker)
    crypto.encrypt(input, encrypted);
    ASSERT_TRUE(encrypted.str() != plaintext);

    // Decrypt (should add marker)
    crypto.decrypt(encrypted, decrypted);

    // Get the decrypted data up to the marker
    std::string decrypted_str = decrypted.str();
    static const std::string EOS_MARKER = "<<DFS_EOS>>";

    // Check if marker exists and is at the end
    size_t marker_pos = decrypted_str.find(EOS_MARKER);
    ASSERT_NE(marker_pos, std::string::npos) << "End of stream marker not found";
    ASSERT_EQ(marker_pos + EOS_MARKER.length(), decrypted_str.length())
        << "Marker should be at the end of stream";

    // Verify content before marker matches original
    ASSERT_EQ(decrypted_str.substr(0, marker_pos), plaintext)
        << "Decrypted content before marker doesn't match input";
}

TEST_F(CryptoStreamTest, LargeStreamWithMarker) {
    std::stringstream input;
    const size_t TEST_SIZE = 1024 * 1024; // 1MB
    std::string expected_data;
    expected_data.reserve(TEST_SIZE);
    ...
    // Find and verify marker
    size_t marker_pos = decrypted_str.find(EOS_MARKER);
    ASSERT_NE(marker_pos, std::string::npos) << "End of stream marker not found in large stream";
    ASSERT_EQ(marker_pos + EOS_MARKER.length(), decrypted_str.length())
        << "Marker should be at the end of large stream";

    // Verify content before marker
    ASSERT_EQ(decrypted_str.substr(0, marker_pos), expected_data)
        << "Decrypted large stream content before marker doesn't match input";
}