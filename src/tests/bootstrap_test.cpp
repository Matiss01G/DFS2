...
TEST_F(BootstrapTest, MultiplePeerFileSharing) {
    const uint8_t PEER1_ID = 1, PEER2_ID = 2, PEER3_ID = 3, PEER4_ID = 4;
    const uint16_t PEER1_PORT = 3001, PEER2_PORT = 3002, PEER3_PORT = 3003, PEER4_PORT = 3004;
    const std::string TEST_FILE_CONTENT = "Test file content for multiple peers";
    const std::string TEST_FILENAME = "multitest.txt";

    std::vector<std::string> peer1_bootstrap_nodes = {};
    std::vector<std::string> peer2_bootstrap_nodes = {ADDRESS + ":3001"};
    std::vector<std::string> peer3_bootstrap_nodes = {ADDRESS + ":3001"};
    std::vector<std::string> peer4_bootstrap_nodes = {ADDRESS + ":3001"};

    Bootstrap peer1(ADDRESS, PEER1_PORT, TEST_KEY, PEER1_ID, peer1_bootstrap_nodes);
    Bootstrap peer2(ADDRESS, PEER2_PORT, TEST_KEY, PEER2_ID, peer2_bootstrap_nodes);
    Bootstrap peer3(ADDRESS, PEER3_PORT, TEST_KEY, PEER3_ID, peer3_bootstrap_nodes);
    Bootstrap peer4(ADDRESS, PEER4_PORT, TEST_KEY, PEER4_ID, peer4_bootstrap_nodes);

    // Start peer1 (bootstrap node) first
    std::thread peer1_thread([&peer1]() {
        ASSERT_TRUE(peer1.start()) << "Failed to start peer 1";
    });
    ...
    std::thread peer4_thread([&peer4]() {
        ASSERT_TRUE(peer4.start()) << "Failed to start peer 4";
    });

    // Allow time for all peers to connect
    std::this_thread::sleep_for(std::chrono::seconds(4));
    ...
    auto& peer4_manager = peer4.get_peer_manager();

    ASSERT_TRUE(peer1_manager.has_peer(PEER2_ID)) << "Peer1 should be connected to Peer2";
    ASSERT_TRUE(peer1_manager.has_peer(PEER3_ID)) << "Peer1 should be connected to Peer3";
    ASSERT_TRUE(peer1_manager.has_peer(PEER4_ID)) << "Peer1 should be connected to Peer4";
    ...
    // Allow time for file propagation
    std::this_thread::sleep_for(std::chrono::seconds(3));

    // Verify file exists in all peers' stores
    auto& file_server2 = peer2.get_file_server();
    auto& file_server3 = peer3.get_file_server();
    auto& file_server4 = peer4.get_file_server();

    ASSERT_TRUE(file_server2.get_store().has(TEST_FILENAME)) 
        << "File should exist in peer2's store after sharing";
    ASSERT_TRUE(file_server3.get_store().has(TEST_FILENAME)) 
        << "File should exist in peer3's store after sharing";
    ASSERT_TRUE(file_server4.get_store().has(TEST_FILENAME)) 
        << "File should exist in peer4's store after sharing";

    // Verify file content matches in all peers
    std::stringstream retrieved_content2, retrieved_content3, retrieved_content4;
    file_server2.get_store().get(TEST_FILENAME, retrieved_content2);
    file_server3.get_store().get(TEST_FILENAME, retrieved_content3);
    file_server4.get_store().get(TEST_FILENAME, retrieved_content4);

    ASSERT_EQ(retrieved_content2.str(), TEST_FILE_CONTENT)
        << "File content mismatch in peer2";
    ASSERT_EQ(retrieved_content3.str(), TEST_FILE_CONTENT)
        << "File content mismatch in peer3";
    ASSERT_EQ(retrieved_content4.str(), TEST_FILE_CONTENT)
        << "File content mismatch in peer4";
...
TEST_F(BootstrapTest, MultiplePeerLargeFileSharing) {
    const uint8_t PEER1_ID = 1, PEER2_ID = 2, PEER3_ID = 3, PEER4_ID = 4;
    const uint16_t PEER1_PORT = 3001, PEER2_PORT = 3002, PEER3_PORT = 3003, PEER4_PORT = 3004;
    const std::string TEST_FILENAME = "large_multitest.txt";
    ...
    // Store large file in peer1
    auto& file_server1 = peer1.get_file_server();
    ASSERT_TRUE(file_server1.store_file(TEST_FILENAME, file_content))
        << "Failed to store large file in peer1";

    // Allow more time for large file propagation
    ...
    ASSERT_TRUE(file_server4.get_store().has(TEST_FILENAME))
        << "Large file should exist in peer4's store after sharing";

    // Verify file size and content matches in all peers
    std::stringstream retrieved_content2, retrieved_content3, retrieved_content4;
    file_server2.get_store().get(TEST_FILENAME, retrieved_content2);
    file_server3.get_store().get(TEST_FILENAME, retrieved_content3);
    file_server4.get_store().get(TEST_FILENAME, retrieved_content4);
    ...
    ASSERT_EQ(retrieved_content2.str().size(), file_content.str().size())
        << "File size mismatch in peer2";
    ASSERT_EQ(retrieved_content3.str().size(), file_content.str().size())
        << "File size mismatch in peer3";
    ASSERT_EQ(retrieved_content4.str().size(), file_content.str().size())
        << "File size mismatch in peer4";

    ASSERT_EQ(retrieved_content2.str(), file_content.str())
        << "Large file content mismatch in peer2";
    ASSERT_EQ(retrieved_content3.str(), file_content.str())
        << "Large file content mismatch in peer3";
    ASSERT_EQ(retrieved_content4.str(), file_content.str())
        << "Large file content mismatch in peer4";
