...
TEST_F(BootstrapTest, BroadcastFileSharing) {
  ...
  const std::string TEST_FILE_CONTENT = "Test broadcast file content";
  const std::string TEST_FILENAME = "broadcast_test.txt";
  ...
  // Start peer1 first as bootstrap node
  std::thread peer1_thread([&peer1]() {
    ASSERT_TRUE(peer1.start()) << "Failed to start peer 1";
  });
  ...
  // Start peer2 and peer3 which will connect to peer1
  std::thread peer2_thread([&peer2]() {
    ASSERT_TRUE(peer2.start()) << "Failed to start peer 2";
  });
  ...
  // Verify all peers are connected to each other
  ASSERT_TRUE(peer1_manager.has_peer(PEER2_ID)) << "Peer1 should be connected to Peer2";
  ...
  // Verify file exists in peer2's and peer3's stores after broadcast
  ASSERT_TRUE(file_server2.get_store().has(TEST_FILENAME)) 
    << "File should exist in peer2's store after broadcast";
  ASSERT_TRUE(file_server3.get_store().has(TEST_FILENAME)) 
    << "File should exist in peer3's store after broadcast";
  ...
  ASSERT_EQ(retrieved_content2.str(), TEST_FILE_CONTENT)
    << "Retrieved file content from peer2 should match original content";
  ...
  ASSERT_TRUE(retrieved_content3.good()) << "Failed to retrieve file from peer3's store";
  ASSERT_EQ(retrieved_content3.str(), TEST_FILE_CONTENT)
    << "Retrieved file content from peer3 should match original content";
...
TEST_F(BootstrapTest, BroadcastLargeFileSharing) {
  const uint8_t PEER1_ID = 1, PEER2_ID = 2, PEER3_ID = 3;
  const uint16_t PEER1_PORT = 3001, PEER2_PORT = 3002, PEER3_PORT = 3003;
  const std::string TEST_FILENAME = "broadcast_large_test.txt";
  ...
  while (current_size < target_size) {
    ...
    // Calculate header size first
    std::string header = "Broadcast-Chunk[" + std::to_string(chunk_number) + 
              "] Position:" + std::to_string(current_size) + " Data:";
    size_t header_size = header.size();
  ...
  // Verify file exists in peer2's and peer3's stores after broadcast
  ASSERT_TRUE(file_server2.get_store().has(TEST_FILENAME)) 
    << "Large file should exist in peer2's store after broadcast";
  ASSERT_TRUE(file_server3.get_store().has(TEST_FILENAME)) 
    << "Large file should exist in peer3's store after broadcast";
  ...
  ASSERT_EQ(retrieved_content2.str().size(), file_content.str().size())
    << "Retrieved file size from peer2 should match original size";
  ASSERT_EQ(retrieved_content2.str(), file_content.str())
    << "Retrieved large file content from peer2 should match original content";
  ...
  ASSERT_EQ(retrieved_content3.str().size(), file_content.str().size())
    << "Retrieved file size from peer3 should match original size";
  ASSERT_EQ(retrieved_content3.str(), file_content.str())
    << "Retrieved large file content from peer3 should match original content";
