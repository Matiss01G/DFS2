...
void Store::store(const std::string& file_key, std::unique_ptr<CryptoStream> data_stream) {
    if (!data_stream) {
...
std::unique_ptr<CryptoStream> Store::get(const std::string& file_key) {
    auto file_path = get_file_path(file_key);
    ...
    // Create a CryptoStream from the file stream
    auto crypto_stream = std::make_unique<CryptoStream>();
    
    // Copy file data to crypto stream
    constexpr size_t buffer_size = 4096;
    char buffer[buffer_size];
    while (stream->read(buffer, buffer_size)) {
        crypto_stream->write(buffer, stream->gcount());
    }
    if (stream->gcount() > 0) {
        crypto_stream->write(buffer, stream->gcount());
    }
